function (self, unitId, unitFrame, envTable, modTable)
    ...
    print("[StarBar VERSION] v13.1 (build 23) -- PERFORMANCE FIX: the entire setup block (version print, GENERATORS/SPENDERS tables, SafePlaySound closure, frame creation) was living OUTSIDE the one-time-build guard, so Plater was re-running/re-allocating all of it on every single hook call (many times a second) instead of just once -- that repeated table/closure allocation and chat spam is what was causing the lag. Moved everything behind a single early-return guard so it only ever runs once, on first load.")
